...
def main():
    ...
    while running:
        ...
        # custom drawing routine with numba
        frame = frameblue.copy()
        draw_triangles(frame, points, triangles, shade, z_order, color_scale)
...
@njit()
def draw_triangles(frame, points, triangles, shade, z_order, color_scale):
    for index in np.argsort(z_order):
        if z_order[index] == 9999: break
        point0 = list(points[triangles[index][0]][3:])
        point1 = list(points[triangles[index][1]][3:])
        point2 = list(points[triangles[index][2]][3:])
        triangle = np.asarray([point0, point1, point2])
        color = (shade[index]*np.abs(points[triangles[index][0]][:3])*color_scale + 25)
        ...
        slope_1, slope_2, slope_3 = 0, 0, 0
        if y_middle != y_start:
            slope_1 = (x_middle - x_start)/(y_middle - y_start)
        if y_stop != y_start:
            slope_2 = (x_stop - x_start)/(y_stop - y_start)
        if y_stop != y_middle:
            slope_3 = (x_stop - x_middle)/(y_stop - y_middle)

        for y in range(max(0, y_start), min(SCREEN_H, y_middle)):
            ...
            frame[minx:maxx, y] = color
        
        for y in range(max(0, y_middle), min(SCREEN_H, y_stop)):
            x2 = x_start + int((y-y_start)*slope_2)
            x3 = x_middle + int((y-y_middle)*slope_3)
            minx, maxx = max(0, min(x3, x2, SCREEN_W)), min(SCREEN_W, max(0, x3,x2))
            frame[minx:maxx, y] = color
